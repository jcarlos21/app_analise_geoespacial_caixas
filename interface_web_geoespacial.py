...
# =========================
# Renderização da seção de resultados usando o estado
# =========================
df_resultado = st.session_state.get("df_resultado")
if df_resultado is not None and not df_resultado.empty:
    st.success("Análise concluída!")

    # >>> Botão para baixar o KMZ único (se existir)
    kmz_path_unico = None
    try:
    ...
    # === DESCRITIVOS (.docx por ponto) + ZIP ===
    st.markdown("### 🧾 Descritivos por ponto (DOCX + Print Satelital)")
    col1, col2, col3, col4 = st.columns([1, 1, 1, 1])

    with col1:
        rota_no_print = st.selectbox(
            "Rotas no descritivo",
            options=["OSRM", "Postes", "Ambas"],
            index=1
        )
    with col2:
        flag_postes_pts = st.checkbox("Incluir marcadores dos postes (pontos)", value=True)
    with col3:
        qualidade_print = st.selectbox(
            "Qualidade do print",
            options=["Padrão", "Alta", "Ultra"],
            index=2,
            help="Ajusta DPI, tamanho, zoom e sobreposição de labels."
        )
    with col4:
        ...
        with st.spinner("Gerando descritivos..."):
            try:
                zip_path = gerar_descritivos_zip(
                    ...
                    rota_no_print=rota_no_print,          # respeita a escolha do usuário
                    qualidade_print=qualidade_print       # presets de qualidade
                )
