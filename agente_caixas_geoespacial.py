...
def _signed_offset(line_utm: LineString, p: Point):
    """
    Retorna:
      s: progresso ao longo da linha (m)
      d: distância lateral absoluta (m)
      d_signed: distância lateral com sinal (m) — >0 à esquerda, <0 à direita,
                considerando o sentido da linha (da caixa para o ponto).
    """
    s = line_utm.project(p)
    # pega ponto na linha e um pouco adiante para estimar tangente local
    on = line_utm.interpolate(s)
    s2 = min(s + 0.5, line_utm.length)  # 0.5m mais à frente para tangente
    on2 = line_utm.interpolate(s2)
    tx, ty = _unit(on2.x - on.x, on2.y - on.y)  # tangente unitária
    # normal "esquerda" (rotacao +90°)
    nx, ny = -ty, tx

    d = p.distance(on)
    # vetor do ponto na linha -> poste
    vx, vy = (p.x - on.x), (p.y - on.y)
    d_signed = vx * nx + vy * ny  # projecao na normal esquerda
    return s, d, d_signed

def _angle(p0, p1, p2):
    """Ângulo interno em graus no ponto p1, formado por p0-p1-p2 (em UTM)."""
...
def gerar_rota_por_postes(
    ...
    coord_caixa=None,
    coord_ponto=None,
    # --- parâmetros das 3 camadas (defaults conservadores) ---
    max_lateral=15.0,           # m (12-15 ruas locais; 20 avenidas)
    ...
    """
    Gera rota aderente a postes com 3 camadas: lado dominante, anti-serrilha e suavização/gaps.
    Mantém assinatura compatível com o app atual.
    """
    ...
    if postes_no_corredor.empty:
        # fallback: conecta caixa->ponto direto
        coords_full_utm = []
    ...
    # fallback por contagem caso mediana ~0
    if abs(med) < 1e-6:
        pos = np.sum(arr[:, 2] > 0)
        neg = np.sum(arr[:, 2] < 0)
        if pos >= neg:
    ...
    # filtro por lado dominante e por offset lateral
    mask_lado = np.sign(arr[:, 2]) == sign_dom
    mask_lateral = arr[:, 1] <= max_lateral
    base_arr = arr[mask_lado & mask_lateral]

    # ---------- 3a) Tolerância a gaps (costurar com lado oposto em janelas longas) ----------
    # se entre s consecutivos houver gap > gap_s, reintroduz alguns postes "originais" (qualquer lado)
    if len(base_arr) >= 2:
        s_base = base_arr[:, 0]
        gaps_idx = np.where(np.diff(s_base) > gap_s)[0]
        for gi in gaps_idx:
            s_a, s_b = s_base[gi], s_base[gi+1]
            # pega candidatos no intervalo (s_a, s_b) respeitando só o max_lateral relaxado (1.2x)
    ...
    # 2a) ângulo máximo entre segmentos + ganho em s mínimo
    def _filtra_angulos(seq_pts, ang_max=angulo_max, ds_min=delta_s_min):
        if len(seq_pts) < 3:
            return seq_pts
        res = [seq_pts[0]]
        for i in range(1, len(seq_pts)-1):
            ...
            cur = seq_pts[i]
            nxt = seq_pts[i+1]
            p0 = (prev[3], prev[4])  # x,y
            p1 = (cur[3], cur[4])
            p2 = (nxt[3], nxt[4])
            ang = _angle(p0, p1, p2)
            ds = nxt[0] - prev[0]  # ganho de s cobrindo o "cotovelo"
            if ang > ang_max and ds < ds_min:
                # descarta cur (serrilha)
                continue
            res.append(cur)
        res.append(seq_pts[-1])
        return res

    seq = _filtra_angulos(seq)

    # 2b) salto lateral máximo entre consecutivos
    def _filtra_salto_lateral(seq_pts, salto_max=salto_lateral_max, ds_min=delta_s_min):
        if len(seq_pts) < 2:
            return seq_pts
        res = [seq_pts[0]]
        for i in range(1, len(seq_pts)):
            prev = res[-1]
            cur = seq_pts[i]
            salto = abs(cur[2] - prev[2])  # usa d_signed p/ comparável a lado
            ds = cur[0] - prev[0]
    ...
    seq = _filtra_salto_lateral(seq)

    # 2c) espaçamento mínimo (thinning) por s
    def _thinning(seq_pts, min_s=espacamento_min):
        if len(seq_pts) < 2:
            return seq_pts
        res = [seq_pts[0]]
        last_s = seq_pts[0][0]
        for i in range(1, len(seq_pts)-1):  # preserva último
    ...
    # Monta coords finais UTM: Caixa -> Postes -> Ponto
    coords_full_utm = []
    if coord_caixa is not None:
        coords_full_utm.append(to_utm.transform(coord_caixa[1], coord_caixa[0]))
    ...
    # ---------- 3b) Suavização (Douglas‑Peucker em UTM) ----------
...
def gerar_mapa_interativo(df_resultados, caminho_html):
    ...
    for _, linha in df_resultados.iterrows():
        ...
        if rota_coords:
            ...
            # se vier no DF (em execuções futuras), desenhamos também
...
def gerar_kmz_unico(nome_base, rotas_info):
    ...
    for item in rotas_info:
        ...
        if rota_postes_coords:
            ...
            # marcadores de postes intermediários (mantido do seu app)
